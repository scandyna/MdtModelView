...
#include "PrependRowTableModel.h"
...
void populateModel(PrependRowTableModel & model, const PrependRowTableModel::Table & tableData)
{
...
{
  PrependRowTableModel model;

  SECTION("prepend row is supported by this model")
  {
...
TEST_CASE("prependRow")
{
  ...
  REQUIRE( getModelData(model, 1, 1) == QLatin1String("A") );
...
TEST_CASE("insertRow_NotSupportedCase")
{
  ...
  SECTION("appending a row is not supported")
  {
  ...
  SECTION("inserting 2 rows at the beginning is not supported")
  {
    REQUIRE( !model.insertRows(0, 2) );
