...
#include "RemoveLastRowTableModel.h"
...
void populateModel(RemoveLastRowTableModel & model, const RemoveLastRowTableModel::Table & tableData)
{
...
TEST_CASE("removeRows_support")
{
  ...
  SECTION("removing the last row is supported by this model")
  {
...
TEST_CASE("removeRow_NotSupportedCases")
{
  ...
  SECTION("remove the first row is not supported")
  {
  ...
  SECTION("remove the 2 last rows is not supported")
  {
    REQUIRE( !model.removeRows(1, 2) );
